...
Java_com_redemastery_oldapi_pojav_Logger_begin(JNIEnv *env, __attribute((unused)) jclass clazz, jstring logPath) {
    if(latestlog_fd != -1) {
    ...
    if(logger_onEventLogged == NULL) {
        jclass eventLogListener = (*env)->FindClass(env, "com/redemastery/oldapi/pojav/Logger$eventLogListener");
...
JNIEXPORT void JNICALL Java_com_redemastery_oldapi_pojav_Logger_appendToLog(JNIEnv *env, __attribute((unused)) jclass clazz, jstring text) {
...
JNIEXPORT void JNICALL
Java_com_redemastery_oldapi_pojav_Logger_setLogListener(JNIEnv *env, __attribute((unused)) jclass clazz, jobject log_listener) {
...
JNIEXPORT void JNICALL
Java_com_redemastery_oldapi_pojav_utils_JREUtils_setupExitMethod(JNIEnv *env, jclass clazz,
                                                        jobject context) {
    exitTrap_ctx = (*env)->NewGlobalRef(env,context);
    (*env)->GetJavaVM(env,&exitTrap_jvm);
    exitTrap_exitClass = (*env)->NewGlobalRef(env,(*env)->FindClass(env,"com/redemastery/oldapi/pojav/ExitActivity"));

...
static void create_hooks(bytehook_hook_all_t bytehook_hook_all_p) {
    ...
    // Only apply chmod hooks on devices where the game directory is in games/MasteryLauncher
...
Java_com_redemastery_oldapi_pojav_utils_JREUtils_initializeHooks(JNIEnv *env, jclass clazz) {

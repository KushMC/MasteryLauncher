...
import com.redemastery.oldapi.pojav.modloaders.FabricVersion;
import com.redemastery.oldapi.pojav.modloaders.FabriclikeDownloadTask;
...
import com.redemastery.oldapi.pojav.modloaders.modpacks.SelfReferencingFuture;
...
public abstract class FabriclikeInstallFragment extends Fragment implements ModloaderDownloadListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void onDataNotAvailable() {
        Tools.runOnUiThread(()->{
            ...
            Tools.dialog(context,
                    ...
                    context.getString(com.redemastery.launcher.R.string.fabric_dl_cant_read_meta, mFabriclikeUtils.getName()));
        });
    }
    ...
    private void updateLoaderVersions() {
        startLoading();
        mLoaderVersionFuture = new SelfReferencingFuture(new LoadLoaderVersionsTask()).startOnExecutor(MasteryLauncherApplication.Companion.getSExecutorService());
    }
    ...
    private void updateGameVersions() {
        startLoading();
        mGameVersionFuture = new SelfReferencingFuture(new LoadGameVersionsTask()).startOnExecutor(MasteryLauncherApplication.Companion.getSExecutorService());
    }
